document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI(state) {
        if (!state || !state.game_in_progress) {
            // ゲーム終了時の処理などをここに書く
            if (state.log && state.log.some(msg => msg.includes("もう一度プレイしますか？"))) {
                clearInterval(gameStateInterval);
                if (confirm("もう一度プレイしますか？")) {
                    startGameBtn.click();
                } else {
                    // Welcome screen or similar
                }
            }
        }
        ...
        const isHumanTurn = state.current_player_index !== null && state.players[state.current_player_index].name === humanPlayer.name;
        
        if (isHumanTurn && state.game_in_progress) {
            enableActionButtons(state.players[state.current_player_index], state.current_bet);
        } else {
